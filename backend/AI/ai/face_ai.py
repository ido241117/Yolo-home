...
def _safe_room_id(room_id: Optional[str]) -> str:
    cleaned = re.sub(r"[^A-Za-z0-9_.-]+", "_", (room_id or "").strip())
    return cleaned.strip("._-")


def face_data_dir(room_id: Optional[str] = None) -> str:
    base = _env_path("FACE_DATA_DIR", DEFAULT_FACE_DATA_DIR)
    safe_room = _safe_room_id(room_id)
    return os.path.join(base, safe_room) if safe_room else base


def face_model_path(room_id: Optional[str] = None) -> str:
    safe_room = _safe_room_id(room_id)
    if not safe_room:
        return _env_path("FACE_MODEL_PATH", DEFAULT_FACE_MODEL_PATH)
    base = _env_path("FACE_MODEL_DIR", os.path.join(BASE_DIR, "models", "faces"))
    return os.path.join(base, safe_room, "face_model.pkl")
...
def _load_model(room_id: Optional[str] = None):
    global _model_cache, _model_signature

    path = face_model_path(room_id)
    mtime = _file_signature(path)
    if mtime is None:
    ...
    if model.get("feature_mode") != FEATURE_MODE or "sample_vectors" not in model:
        train_face_model(room_id=room_id)
        sig = (path, _file_signature(path))
        model = joblib.load(path)
    _model_cache = model
...
def train_face_model(room_id: Optional[str] = None) -> dict:
    data_dir = face_data_dir(room_id)
    model_path = face_model_path(room_id)
...
def list_registered_faces() -> list:
    return list_registered_faces_for_room()


def list_registered_faces_for_room(room_id: Optional[str] = None) -> list:
    data_dir = face_data_dir(room_id)
    model = None
    try:
        model = _load_model(room_id)
    except Exception:
        model = None
...
def delete_registered_face(label: str, auto_retrain=True, room_id: Optional[str] = None) -> dict:
    safe_label = _safe_label(label)
    if not safe_label:
        raise ValueError("missing_label")

    label_dir = os.path.join(face_data_dir(room_id), safe_label)
    data_root = os.path.abspath(face_data_dir(room_id))
    ...
    retrain_result = None
    remaining_faces = list_registered_faces_for_room(room_id)
    model_path = face_model_path(room_id)
...
def register_face(label: str, image_data=None, image_file=None, auto_retrain=True, room_id: Optional[str] = None) -> dict:
    safe_label = _safe_label(label)
    if not safe_label:
        raise ValueError("missing_label")
...
def recognize_face_image(image_data=None, image_file=None, room_id: Optional[str] = None, open_door=False) -> dict:
    try:
        data = _decode_image(image_data=image_data, image_file=image_file)
        validate_face_quality(data)
